...
//						}
	...
	private JComboBox getCbShopid() {
		if (cbShopid == null) {
			...
			// cbShopid.setModel();
			// cbShopid.setModel(new DefaultComboBoxModel(new String[] {"사원", "점장",
			// "관리자"}));
	...
	//
	// Integer.parseInt(tfStaffId.getText()), tfStaffName.getText(),
	// String.valueOf(pwfPassword.getPassword()), tfPhone.getText(),
	// Integer.parseInt(tfHourlyWage.getText()),
	// cbShopid.getItemAt(cbShopid.getSelectedIndex()).toString()
	// ,positionInsert());
	// 시원정보 입력
	...
	private void conditionQueryAction(String conditionQueryColumn) {
		...
		for (int index = 0; index < listCount; index++) {
			...
			String[] qTxt = { temp, dtoList.get(index).getPosition(), dtoList.get(index).getBelog(),
					dtoList.get(index).getStaffname(), dtoList.get(index).getStaffphone(),
					Integer.toString(dtoList.get(index).getStaffhourlywage()), dtoList.get(index).getStaffinitdate(),
					dtoList.get(index).getStaffdeletedate() };
